...
export class Display {
  ...
  constructor(
    ...
    // prettier-ignore
    this.bindGroups = [
      this.createBindGroup(sim.buffers.particles1),
      this.createBindGroup(sim.buffers.particles2),
    ]
  }
  ...
  private createPaletteBuffer() {
    ...
    const colors = new Float32Array(
      Array.from({ length: this.sim.settings.colors }, (_, i) => [...hsvToRgb((i / this.sim.settings.colors) * 360 + offset, s, v), 0]).flat(),
    )
  ...
  render(encoder: GPUCommandEncoder, step: number) {
    ...
    pass.drawIndexed(this.geometry.numVertices, this.sim.settings.count)
    pass.end()
  }
}
