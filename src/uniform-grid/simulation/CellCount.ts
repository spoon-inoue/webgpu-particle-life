...
import shader from './cellCount.wgsl?raw'

export class CellCount extends UniformGrid {
  private readonly pipeline: GPUComputePipeline
  private readonly bindGroupLayout: GPUBindGroupLayout
  private bindGroup: GPUBindGroup

  constructor(device: GPUDevice, buffers: Buffers, settings: Settings) {
    super(device, buffers, settings, 'CellCount')
  ...
  private createBindGroup() {
    return this.device.createBindGroup({
      ...
      entries: [
        ...
        { binding: 1, resource: this.buffers.cellCount },
      ],
    })
  }

  compute(encoder: GPUCommandEncoder) {
    encoder.clearBuffer(this.buffers.cellCount)
